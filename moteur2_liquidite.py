...
class Moteur2Liquidite:
    ...
    def analyser(
        ...
        raw_levels = self._extract_levels(
            market_map,
            current_price,
        )
        if not raw_levels:
        ...
        return {
            ...
            "buy_side_liquidity": [
                asdict(item)
                for item in buy_levels
            ],
            "sell_side_liquidity": [
                asdict(item)
                for item in sell_levels
            ],
            "liquidity_levels": [
                asdict(item)
                for item in levels
            ...
            "sweep_areas": [
                asdict(item)
                for item in sweep_areas
            ],
            "sweep_clusters": [
                asdict(item)
                for item in sweep_clusters
            ],
    ...
    def _zone_center(
        ...
        if center is not None and center > 0:
            return center
        low = Moteur2Liquidite._safe_float(
            zone.get("low")
        )
        high = Moteur2Liquidite._safe_float(
            zone.get("high")
        )
    ...
    def _build_level(
        ...
        if sweep_candidate:
            reason_parts.append(
                "zone de recherche de liquidité potentielle"
            )
        if touches > 1:
            reason_parts.append(
                f"{touches} réactions"
        ...
        for index, level in enumerate(levels):
            ...
            clusters.append(
                LiquidityCluster(
                    ...
                    sweep_candidate=sweep_candidate,
                    reason=(
                        f"Cluster {level.side} "
                        f"de {len(group)} niveaux "
    ...
    def _build_summary(
        ...
        if nearest_buy and nearest_sell:
            if (
                nearest_buy.distance_percent
                < nearest_sell.distance_percent
            ):
                dominant_side = (
                    "BUY_SIDE_PLUS_PROCHE"
                )
            elif (
                nearest_sell.distance_percent
                < nearest_buy.distance_percent
    ...
    def _build_liquidity_possibilities(
        ...
        """
        Construit des possibilités de lecture sans les transformer
        en signaux.
        Cette méthode reste strictement descriptive et non bloquante.
        """
        ...
        sweep_near = [
            x
            for x in levels
            if x.sweep_candidate
        ]
        if nearby_buy and nearby_sell:
            possibilities.append({
                "type": (
                    "LIQUIDITY_BALANCE_NEAR_PRICE"
                ),
                "description": (
                    "Des niveaux sont présents des "
                    "deux côtés du prix ; une interaction "
                    "avec l'un ou l'autre côté mérite "
                    "une surveillance."
                ),
                "evidence": {
                    "buy": len(nearby_buy),
                    "sell": len(nearby_sell),
                },
                "confidence": 55.0,
            })
            ...
            possibilities.append({
                "type": (
                    "LIQUIDITY_SIDE_NEAR_PRICE"
                ),
                "description": (
                    f"Une concentration de liquidité "
                    f"potentielle est proche du prix "
                    f"sur {side}."
                ),
                "evidence": {
                    "side": side,
        ...
        if strong_clusters:
            possibilities.append({
                ...
                "description": (
                    "Plusieurs niveaux se regroupent "
                    "dans une même zone de prix ; cette "
                    "concentration peut devenir un point "
                    "d'interaction important."
                ),
                "evidence": {
                    "clusters": len(
                        strong_clusters
        ...
        if buy_clusters and sell_clusters:
            possibilities.append({
                ...
                "description": (
                    "La cartographie montre des "
                    "concentrations potentielles au-dessus "
                    "et sous le prix, laissant plusieurs "
                    "trajectoires d'interaction ouvertes."
                ),
                "evidence": {
                    "buy_clusters": len(
                        buy_clusters
        ...
        if sweep_near:
            possibilities.append({
                "type": (
                    "LIQUIDITY_INTERACTION_WATCH"
                ),
                "description": (
                    "Des niveaux sont suffisamment proches "
                    "pour surveiller une prise de liquidité "
                    "ou une réaction du prix ; aucune "
                    "occurrence n'est affirmée à l'avance."
                ),
                "evidence": {
                    "candidates": len(
                        sweep_near
        ...
        if not possibilities:
            possibilities.append({
                "type": (
                    "LIQUIDITY_DISTRIBUTION_OPEN"
                ),
                "description": (
                    "La liquidité identifiée est dispersée ; "
                    "aucune concentration locale ne domine "
        ...
        #
        # Cette ligne provoquait :
        #
        # TypeError:
        # Moteur2Liquidite._get_average_range()
        # missing 1 required positional argument: 'timeframe'
        #
        # La méthode _get_average_range() exige en effet :
        #
        #     _get_average_range(market_map, timeframe)
        #
        # Mais ici, chaque LiquidityLevel possède déjà un
        # distance_units calculé avec l'amplitude moyenne
        # de SON propre timeframe dans _build_level().
        #
        # Nous réutilisons donc directement cette information.
        # Cela évite de choisir arbitrairement un timeframe et
        # conserve une lecture multi-timeframes cohérente.
        # ==================================================================
        if levels:
            valid_levels = [
                x
                for x in levels
                if x.distance_units > 0
            ]
            if valid_levels:
                nearest = min(
                    valid_levels,
                    key=lambda x: x.distance_units,
                )
                relative_distance = (
                    nearest.distance_units
                )
                if relative_distance <= 1.5:
                    possibilities.append({
                        "type": (
                            "LIQUIDITY_WITHIN_RECENT_RANGE"
                        ),
                        "description": (
                            "Le niveau de liquidité le plus "
                            "proche se situe dans une distance "
                            "comparable à l'amplitude récente "
                            "de son timeframe."
                        ),
                        "evidence": {
                            "distance_vs_range": round(
                                relative_distance,
                                3,
                            ),
                            "timeframe": (
    ...
    def _build_adaptive_reading(
        ...
        if not levels:
            ...
            state = (
                "LIQUIDITY_HIGHLY_CONCENTRATED"
            )
        elif len(clusters) >= 1:
            state = (
                "LIQUIDITY_CONCENTRATED"
            )
        else:
            state = (
                "LIQUIDITY_DISTRIBUTED"
            )
        return {
            "state": state,
            "level_count": len(levels),
            "cluster_count": len(clusters),
            "possibility_count": len(
                possibilities
            ),
            "interpretive": True,
            "blocking": False,
    ...
    def _interpret_liquidity(
        ...
        if (
            nearest_buy is None
            and nearest_sell is None
        ):
            return (
                "Aucun niveau de liquidité exploitable "
                "identifié."
            )
        if (
            ...
            if (
                ...
                base = (
                    "La liquidité potentielle supérieure "
                    "est actuellement plus proche du prix."
                )
            elif (
                nearest_sell.distance_percent
                < nearest_buy.distance_percent
                ...
                base = (
                    "La liquidité potentielle inférieure "
                    "est actuellement plus proche du prix."
                )
            else:
                base = (
                    "Les deux côtés de la liquidité sont "
                    "à distance comparable."
                )
        elif nearest_buy is not None:
            base = (
                "Le niveau de liquidité potentielle "
                "supérieur le plus proche est celui "
                "identifié par la cartographie."
            )
        else:
            base = (
                "Le niveau de liquidité potentielle "
                "inférieur le plus proche est celui "
                "identifié par la cartographie."
            )
        if any(
            cluster.level_count >= 2
            for cluster in clusters
        ):
            base += (
                " Des concentrations multi-niveaux "
    ...
    def _get_average_range(
        ...
        """
        Retourne l'amplitude moyenne du timeframe demandé.
        Cette méthode exige volontairement le timeframe afin de ne pas
        mélanger les amplitudes H4, H1, M15, M5 et M1.
        """
        timeframes = market_map.get(
            "timeframes",
            {},
    ...
    def _empty_result(
        ...
        return {
            ...
            "summary": {
                ...
                "interpretation": (
                    "Aucune donnée de prix exploitable "
                    "pour analyser la liquidité."
                ),
            },
            "descriptive_only": True,
            "blocking": False,
