...
Fonctionnalites :
    - selection manuelle de la paire
    - analyse de la paire selectionnee
    - actualisation de la derniere paire selectionnee
    - scanner automatique independant sur toutes les paires
    - analyses serialisees
    - test temporaire du superviseur economique
"""
...
async def help_command(
    ...
    text = (
        ...
        "/about — Informations\n"
        "/help — Aide\n"
        "/testnews — Test temporaire des annonces économiques"
    )
...
async def test_news_command(
    ...
    """
    Test temporaire du Economic News Supervisor.

    IMPORTANT :
    Cette commande est strictement informative.

    Elle ne :
        - lance aucune analyse de marché
        - ne génère aucun BUY/SELL
        ...
        - ne modifie pas le moteur de trading
    """
    ...
    try:
        ...
        if not events:
            ...
            await update.message.reply_text(
                "❌ *TEST ÉCHOUÉ*\n\n"
                "Aucune annonce économique HIGH "
                "n'a été récupérée.\n\n"
                "Vérifie que `FINNHUB_API_KEY` "
                "est bien présente dans Railway.",
                parse_mode="Markdown",
            )
