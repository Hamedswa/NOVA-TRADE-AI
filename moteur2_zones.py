...
MAX_LIQUIDITY_SCORE = 8.0
LIQUIDITY_PROXIMITY_RATIO = 0.35
# ---------------------------------------------------------------------------
# STRUCTURES
# ---------------------------------------------------------------------------
@dataclass
class ZoneCandidate:
    ...
    liquidity_relation: str
    liquidity_distance_units: float
    liquidity_cluster_score: float
...
class Moteur2Zones:
    ...
    def _qualify_zone(
        ...
        # LIQUIDITÉ
        # ---------------------------------------------------------------
        # La liquidité enrichit la qualification d'une zone mais ne peut
        # jamais retirer de points. Elle reste descriptive et non
        # décisionnelle : aucune direction, aucun signal, aucun blocage.
        liquidity_info = self._evaluate_liquidity_relation(
            zone=zone,
            liquidity=liquidity,
            market_map=market_map,
            current_price=current_price,
        )
        liquidity_score = liquidity_info["score"]
        liquidity_relation = liquidity_info["relation"]
        liquidity_distance_units = liquidity_info["distance_units"]
        liquidity_cluster_score = liquidity_info["cluster_score"]
        ...
        return ZoneCandidate(
            ...
            liquidity_relation=liquidity_relation,
            liquidity_distance_units=round(
                liquidity_distance_units,
                4,
            ),
            liquidity_cluster_score=round(
                liquidity_cluster_score,
                2,
            ),
    ...
    # LIQUIDITÉ
    # -----------------------------------------------------------------------
    def _evaluate_liquidity_relation(
        self,
        zone: Dict[str, Any],
        ...
        """
        Mesure le lien descriptif entre une zone et la cartographie de
        liquidité déjà produite par moteur2_liquidite.py.

        Cette méthode ne transforme jamais une proximité en décision
        directionnelle. Elle ajoute uniquement un petit bonus de
        qualification lorsque la zone est proche d'un niveau ou d'un
        cluster de liquidité.
        """
        result = {
            "score": 0.0,
            "relation": "none",
            "distance_units": 999.0,
            "cluster_score": 0.0,
        }
        if not isinstance(liquidity, dict):
            return result

        center = self._safe_float(zone.get("center"), None)
        if center is None or center <= 0:
            return result

        timeframe = str(zone.get("timeframe", "")).upper()
        volatility = self._estimate_volatility(
            market_map=market_map,
            timeframe=timeframe,
            reference_price=center,
        ...
        if volatility <= 0:
            volatility = max(center * MIN_VOLATILITY_RATIO, 1e-12)

        best_level_distance = float("inf")
        best_cluster_distance = float("inf")
        best_cluster_score = 0.0

        def inspect_levels(values: Any) -> None:
            nonlocal best_level_distance
            if not isinstance(values, list):
                return
            for item in values:
                if isinstance(item, dict):
                    value = item.get("price", item.get("level", item.get("center")))
                else:
                    value = item
                price = self._safe_float(value, None)
                if price is None or price <= 0:
                    continue
                distance = abs(center - price) / volatility
                if distance < best_level_distance:
                    best_level_distance = distance

        def inspect_clusters(values: Any) -> None:
            nonlocal best_cluster_distance, best_cluster_score
            if not isinstance(values, list):
                return
            for cluster in values:
                if not isinstance(cluster, dict):
                    continue
                cluster_price = self._safe_float(
                    cluster.get("center", cluster.get("price")),
                    None,
                )
                if cluster_price is None or cluster_price <= 0:
                    continue
                distance = abs(center - cluster_price) / volatility
                raw_score = self._safe_float(
                    cluster.get("score", cluster.get("cluster_score", 0.0)),
                    0.0,
                ) or 0.0
                normalized_score = max(0.0, min(raw_score / 100.0, 1.0))
                if distance < best_cluster_distance:
                    best_cluster_distance = distance
                    best_cluster_score = normalized_score
        ...
        for key in (
            ...
            inspect_levels(liquidity.get(key))
        ...
        for key in (
            ...
            inspect_clusters(liquidity.get(key))

        # Les niveaux proches apportent le signal descriptif principal.
        if best_level_distance <= 0.75:
            level_score = 4.0 * (1.0 - best_level_distance / 0.75)
            result["score"] += max(0.0, min(level_score, 4.0))
            result["relation"] = "near_liquidity"
            result["distance_units"] = best_level_distance
        elif best_level_distance <= 1.50:
            level_score = 1.5 * (1.0 - (best_level_distance - 0.75) / 0.75)
            result["score"] += max(0.0, min(level_score, 1.5))
            result["relation"] = "liquidity_context"
            result["distance_units"] = best_level_distance

        if best_cluster_distance <= 1.00:
            cluster_bonus = 4.0 * (1.0 - best_cluster_distance / 1.00)
            cluster_bonus *= 0.5 + 0.5 * best_cluster_score
            result["score"] += max(0.0, min(cluster_bonus, 4.0))
            result["relation"] = "near_cluster" if result["relation"] == "none" else result["relation"]
            if best_cluster_distance < result["distance_units"]:
                result["distance_units"] = best_cluster_distance
            result["cluster_score"] = best_cluster_score * 100.0

        result["score"] = max(0.0, min(result["score"], MAX_LIQUIDITY_SCORE))
        return result
    ...
    def _build_reason(
        ...
        if liquidity_relation == "near_liquidity":
            reasons.append("proche d'une zone de liquidité")
        elif liquidity_relation == "near_cluster":
            reasons.append("proche d'un cluster de liquidité")
        elif liquidity_relation == "liquidity_context":
            reasons.append("contexte de liquidité présent")
        return " + ".join(
            reasons
        )
    # -----------------------------------------------------------------------
